...
public class AutomationPracticeStepDefinitions {
    ...
    public void reference_alanini_temizler() {
        automationPracticepage.additionalInformation.clear();
    }
    @Then("reference yazar")
    public void reference_yazar() {
        automationPracticepage.referance.sendKeys(ConfigReader.getProperty("Ap_referance"));
    }
    ...
    Assert.assertEquals(ConfigReader.getProperty("Ap_hata_mesaji"), automationPracticepage.hatatext.getText());
    }

    @And("ap email kutusuna gecersiz email bilgisi yazar ve enter'e basar")
    public void apEmailKutusunaGecersizEmailBilgisiYazarVeEnterEBasar() {
        automationPracticepage.emailTextBox.sendKeys(ConfigReader.getProperty("Ap_gecersiz_email")+Keys.ENTER);

    }

    @And("ivalid password ile girilemedigini test eder")
    public void ivalidPasswordIleGirilemediginiTestEder() {
